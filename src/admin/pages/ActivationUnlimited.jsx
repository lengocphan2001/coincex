...
import { FaSearch, FaSpinner, FaPhone, FaClock, FaCheck, FaTimes } from 'react-icons/fa';
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-white">Gói kích hoạt thành viên</h1>
                ...
                users.map((user) => (
                  <tr key={user.user_id} className="hover:bg-[#2C2C2C] transition-colors duration-150">
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-white">
                      <div className="flex items-center gap-2">
                        <FaPhone className="text-gray-400 sm:hidden" />
                        {user.phone_number}
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-white">
                    ...
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm">
                      <FaCheck className={`sm:hidden ${user.active_unlimitted ? 'text-green-400' : 'text-gray-400'}`} />
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${user.active_unlimitted
                        ? 'bg-green-900/30 text-green-400'
                        : 'bg-gray-800 text-gray-400'
                        }`}>
                        {user.active_unlimitted ? 'Đã kích hoạt' : 'Chưa kích hoạt'}
                      </span>
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm">
                      <button
                        onClick={() => {
                          ...
                          setActionType(user.active_unlimitted ? 'deactivate' : 'activate');
                          setShowModal(true);
                        }}
                        className={`${user.active_unlimitted
                          ? 'text-red-400 hover:text-red-300'
                          : 'text-[#00DC82] hover:text-[#00b86c]'
                          } transition-colors duration-200`}
                      >
                        {user.active_unlimitted ? 'Hủy' : 'Kích hoạt'}
                      </button>
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1E1E1E] rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
            <h3 className="text-xl font-bold text-white mb-4">
              Xác nhận {actionType === 'activate' ? 'kích hoạt' : 'hủy kích hoạt'} thành viên vĩnh viễn
            </h3>
            <p className="text-gray-300 mb-6">
              Bạn có chắc chắn muốn {actionType === 'activate' ? 'kích hoạt' : 'hủy kích hoạt'} thành viên {selectedUser}?
            </p>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const token = localStorage.getItem('adminToken');
                    await axios.put(
                      `http://localhost:5001/api/users/${selectedUser}/unlimited-status`,
                      { active_unlimitted: actionType === 'activate' ? 1 : 0 },
                      { headers: { Authorization: `Bearer ${token}` } }
                    );
                    setUsers(users.map(u =>
                      u.user_id === selectedUser
                        ? { ...u, active_unlimitted: actionType === 'activate' ? 1 : 0 }
                        : u
                    ));
                    setShowModal(false);
                    setSelectedUser(null);
                    ...
                    setError('Failed to update unlimited status');
                  } finally {
                    setLoading(false);
                  }
                }}
                className={`px-4 py-2 rounded-lg ${actionType === 'activate'
...
export default ActivationUnlimited; 